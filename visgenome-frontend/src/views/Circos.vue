      ...
      <el-container style="height: 100%">
        ...
        <el-main class="insert">
          <div class="tips">
            ...
            circle, oriented out, and the other is the same plot, but inside the
            circle, origented in. Every tick represents 5 units of resolution.
            The following example circular image is with the resolution set to
            1.
          </div>
          ...
          >
          <!-- <div class="tips">There are two plots in the circular image, one is outside the circle, oriented out, and the other is the same plot, but inside the circle, origented in.</div> -->
          <img
          ...
          <img
            ...
            alt="circos image"
          />
          <img :src="circos" :alt="circos" v-show="showImg" class="circosImg" />
        </el-main>
      </el-container>
    </el-container>
  </div>
...
export default {
  ...
  mounted() {
    ...
    // 如果有circosState缓存，则表示是刷新页面之前缓存下来的数据，否则是从Tracks页面或者直接从菜单栏点击过来的，还需要监听路由变化，然后清除缓存
